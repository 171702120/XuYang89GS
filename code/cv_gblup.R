...
cv_gblup<- function(fix = NULL, y, kk, nfold = 10,cpu=5,seed=1) {
  n <- length(y)
  y <- as.matrix(y)
  if (is.null(fix)) {
    fix <- matrix(1, n, 1)
  ...
  mixed<-function (fix = NULL, y, kk) {
    ...
    nloglik_REML <- function(pm) {
      ...
      nloglik <- 0.5 * (unlist(determinant(v))[[1]] + unlist(determinant(t(fix) %*%v_i %*% fix))[[1]] + t(y - fix %*% beta) %*% v_i %*%(y - fix %*% beta))
      return(nloglik)
    }
    parm0 <- rep(1, g + 1)
    parm <- optim(par = parm0, fn = nloglik_REML, method = "L-BFGS-B",lower=0,hessian = FALSE)
    v_phi <- 0
    for (p in 1:g) {
